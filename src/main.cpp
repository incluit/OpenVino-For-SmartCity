...
std::string return_current_time_and_date()
{
    ...
    std::stringstream ss;
    ss << std::put_time(std::localtime(&in_time_t), "%Y-%m-%d_%X"); // Format
    return ss.str();                                                // Return the datetime
}
...
void init_logging(std::string base)
{
    ...
    fileName << "log/" << base << "_" << return_current_time_and_date() << ".log"; // Filename with current datetime
...
int main(int argc, char *argv[])
{
    ...
    {
        // ---------------------------Init Log-------------------------------
        init_logging("test"); // Call to logging with the file name "test"
        // ---------------------------Parsing and validation of input args--------------------------------------
...
// -------------------------- DATABASE TO SHOW THE RESULTS --------------------------
#ifdef ENABLED_DB
        if (FLAGS_show_graph)
        { // If the argument show_graph was defined, try execute MongoDB
        ...
        cv::VideoCapture cap; // Class of OpenCV for video capturing from video files, image sequences or cameras.
        if (!(FLAGS_i == "cam" ? cap.open(0) : cap.open(FLAGS_i)))
        { // Open the camera or the file indicated in the argument
        ...
        for (auto &&option : cmdOptions)
        {
            ...
            {
#if (OPENVINO_VER == 2019) // Check if the OpenVino's version is 2019 or other
